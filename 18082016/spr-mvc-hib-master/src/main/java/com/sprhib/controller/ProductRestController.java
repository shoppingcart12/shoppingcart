...
public class ProductRestController {
	 ...
	 * ResponseEntity checkLogin(@ModelAttribute Users loginBean) { boolean
	 * status = userService.checkLoginInf(loginBean.getUsername(),
	 * loginBean.getPassword()); if(!status){ //return new
	 * ResponseEntity<List<Users>>(HttpStatus.NO_CONTENT);//You many decide to
	 * return HttpStatus.NOT_FOUND return new ResponseEntity("login fail",
	 * HttpStatus.OK); } else { return new
	 * ResponseEntity("login success :Login user :"+loginBean.getUsername(),
	 * HttpStatus.OK); } }
	 */
	...
	public void checkLogin(@RequestParam("username") String username,
		...
		if (!status) {
			// return new
			// ResponseEntity<List<Users>>(HttpStatus.NO_CONTENT);//You many
	...
	public ResponseEntity<Void> addProduct(@ModelAttribute Product product,
		...
		headers.setLocation(ucBuilder.path("/product/{productid}")
	 ...
	 * headers = "Accept=application/json") public ResponseEntity<Product>
	 * addProducts(@RequestBody Product product) {
	 * 
	 * return new ResponseEntity<Product>(productService.addProduct(product),
	 * HttpStatus.OK); }
	 */

	@RequestMapping(value = "/addproducts", method = RequestMethod.GET)
	public void addProducts(@RequestParam("pname") String pname,
			@RequestParam("pbrand") String pbrand,
			@RequestParam("pprice") String pprice, HttpServletResponse res) {
		PrintWriter out = null;
		try {
			out = res.getWriter();
		} catch (Exception e) {
		}
		int prices = Integer.parseInt(pprice);
		Product product = new Product();
		product.setBrand(pbrand);
		product.setProductname(pname);
		product.setPrice(prices);
		product.setImgpath("a.jpg");
	...
	public ResponseEntity<List<Product>> getProductByBrand(
		...
		return new ResponseEntity<List<Product>>(
				productService.searchProduct(brand), HttpStatus.OK);
	}

	@RequestMapping(value = "/getbrands", method = RequestMethod.GET, headers = "Accept=application/json")
	public ResponseEntity<List<String>> getBrands() {
		return new ResponseEntity<List<String>>(productService.getBrands(),
				HttpStatus.OK);
	}
}
